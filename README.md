...
새 퀘스트는 파일 하나로 추가한다. 아레나는 별도 실행 모드에서 **도전 → 결과 확인 → 재도전**을
멈출 때까지 반복한다.
...
아래 두 가지를 실제로 하는 과정을 처음부터 끝까지 녹화해 두었다.
글로 읽기보다 한 번 보는 편이 빠르다.

**▶ [전체 가이드 영상 (3분 30초)](docs/setup-guide.mp4)** — 설치부터 실행까지 전 과정.
...
**▶ [실행 화면만 (1분 9초)](docs/demo.mp4)** — 위 전체 영상에서 설정 과정을 뺀 부분.
시작을 누른 뒤 퀘스트를 알아보고 수행하는 과정만 담겨 있다.
...
| 퀘스트 재개 | 대기에서 다시 시작한다 |
| 아레나 시작 / 아레나 종료 | 아레나 재도전만 별도 모드로 켜고 끈다 |

오른쪽 로그 패널에 최근 100줄이 남는다.

### 아레나 무한 도전

실행 탭의 **아레나 시작**을 누르면 일반 자동화와 섞이지 않는 전용 모드가 시작된다.
도전 준비 화면의 **도전하기**를 누르고, 전투가 끝나면 결과 화면을 ESC로 닫은 뒤 다시
도전한다. **아레나 종료**를 누를 때까지 같은 흐름을 반복한다.

일반 자동화가 도는 중에 **아레나 시작**을 누르거나, 아레나가 도는 중에 **시작**을 누르면
현재 모드에 먼저 정지를 요청한다. 작업 스레드가 끝난 것을 확인한 뒤 선택한 모드로 자동
전환하므로 종료 버튼을 따로 누를 필요는 없다.

처음 실행하기 전에 설정 탭의 **템플릿 설정**에서 다음 두 조각을 캡처한다.

- 도전 준비 화면 — 주황 버튼 안의 `도전하기` 글자 주변
- 결과 화면 — 아래쪽의 고정 문구 `화면을 탭하세요` 주변

점수 · 순위 · 보상처럼 매번 바뀌는 부분은 결과 화면 템플릿에 넣지 않는다. 게임 화면
이미지는 다른 템플릿과 마찬가지로 `local/templates/`에만 저장되고 git에는 들어가지 않는다.
...
| 눌러야 할 주황 버튼 | 아래쪽 띠에서 색이 꽉 찬 판을 찾음 | 버튼 채움 0.96~0.97 / 배경 잡동사니 0.42~0.57 |
...
읽었거나 마침 퀘스트가 바뀌는 중일 수 있다. 다시 보기 전에 `빈 곳 탭 지점`을 한 번 누른다 —
연출이나 말풍선이 이름 줄만 살짝 덮으면 색은 그대로라 창은 보이는데 글자가 안 맞기 때문이다.
누르는 자리는 버튼이 없는 전장 한복판이라 가린 게 없었으면 아무 일도 없다(X 가 보이면 누르지
않는다. 그런 팝업은 빈 곳으로 안 닫힌다). **30초**를 채우면 퀘스트확인부터 다시 밟고, 그렇게
**3라운드**(총 1분 30초)를 되풀이해도 못 알아보면 알림 후 대기하고 그 화면을
`local/captures/` 에 저장한다. 중간에 한 번이라도 알아보면 라운드 수는 0 으로 돌아간다.

**장비가 가득 찼을 때.** 장비가 30개(최대)면 뽑은 것을 놓을 자리가 없어 자동 열기가 더 돌지
않는다. 이때는 Auto 팝업에 '시작' 대신 **'정리하기'** 가 뜬다. 그 자리의 버튼은 글자가 아니라
색으로 찾으므로 무엇이 뜨든 그대로 눌린다. '정리하기' → 결과 목록의 '정리 하기' → 확인창의
'정리하기' 까지 눌러 자리를 만들고 다시 뽑는다. 전투력이 오르는 장비만 남고 나머지는 판매된다.
...
한국어 Gherkin 으로 쓴 BDD 시나리오 51개다. `tests/features/*.feature` 를 읽으면 퀘스트와 아레나가
