...
아래 두 가지를 실제로 하는 과정을 녹화해 두었다. 글로 읽기보다 한 번 보는 편이 빠르다.

**▶ [설정 가이드 영상 (3분 30초)](docs/setup-guide.mp4)**

> 영상 속 게임 계정 정보(프로필 · 레벨 · 닉네임 · 순위 · 전투력 · 길드명)는 검게 가려 두었다.
> Git LFS 로 관리하므로, 저장소를 받을 때 파일이 아니라 포인터만 받았다면
> `git lfs pull` 로 내려받는다.
...
| 어떤 퀘스트인가 | 이름을 이진화해 템플릿 매칭, 최고점 하나를 고름 | 등록 0.68~0.99 / 미등록 0.36~0.37 |
...
읽었거나 마침 퀘스트가 바뀌는 중일 수 있다. **30초**를 채우면 퀘스트확인부터 다시 밟고, 그렇게
**3라운드**(총 1분 30초)를 되풀이해도 못 알아보면 알림 후 대기하고 그 화면을
`local/captures/` 에 저장한다. 중간에 한 번이라도 알아보면 라운드 수는 0 으로 돌아간다.
...
임계값(글자 0.55 / 그림 0.80)을 넘는지 반드시 확인한다. 넘지 못하면 템플릿이 너무 작은
것이므로 더 넓게 다시 잡는다.
