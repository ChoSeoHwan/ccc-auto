...
BUTTON_ORANGE = ((13, 150, 180), (25, 255, 255))
...
**색상 하한이 8 이 아니라 13 인 이유.** 하단 닫기(X) 버튼은 붉은 주황이라
8 로 두면 같이 걸린다. 화면이 위로 밀리면 X 가 버튼 띠 아래끝에 걸치는데,
그러면 덩어리를 메우는 과정에서 버튼과 X 가 **한 덩어리로 이어져** 경계
사각형이 216x66 으로 부풀고 채움이 0.724 로 떨어져 버튼을 놓친다. 실제로
자동화가 '정리하기' 확인창 앞에서 재료 부족으로 오진하고 멈췄다.

    정리하기 버튼   H 18 (5%~95% 전부 18)
    X 버튼          H 8  (띠에 걸치는 윗부분은 7~8)

둘이 확실히 떨어져 있어 그 가운데를 잡았다.
"""
